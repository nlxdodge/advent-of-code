...
def reset_code(op_codes_default, op_codes)
  op_codes = op_codes_default.clone
end

def read_code(op_codes, index)
  op_codes[op_codes[index]]
end

def write_code(op_codes, index, code)
  op_codes[op_codes[index]] = code
end

def do_calculation(op_codes, index, times = false)
  nr1 = read_code(op_codes, index + 1)
  nr2 = read_code(op_codes, index + 2)
  if !times
    write_code(op_codes, index + 3, nr1 + nr2)
  else
    write_code(op_codes, index + 3, nr1 * nr2)
  end
end

def check_instruction_set(op_codes)
  (0..(op_codes.length - 1)).step(4).each do |index|
    case op_codes[index]
    when 1
      do_calculation(op_codes, index)
    when 2
...
op_codes[1] = 12
op_codes[2] = 2
puts "Answer of part 1: #{check_instruction_set(op_codes)}"
reset_code(op_codes_default, op_codes)
...
(0..99).each do |noun|
  (0..99).each do |verb|
    op_codes[1] = noun
    op_codes[2] = verb
    output = check_instruction_set(op_codes)
